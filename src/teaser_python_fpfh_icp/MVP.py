...
random_pair_num = 300#np.random.randint(0,1000)
cloud = o3d.geometry.PointCloud()
cloud.points = o3d.utility.Vector3dVector(copy.deepcopy(src_cloud[random_pair_num,:,:].reshape(-1,3)))
transformed_cloud = o3d.geometry.PointCloud()
transformed_cloud.points = o3d.utility.Vector3dVector(copy.deepcopy(complete_cloud[random_pair_num,:,:].reshape(-1,3)))
...
# transformed_cloud = copy.deepcopy(transformed_cloud).translate(-center_of_cloud)
...
translation = np.array([0,0,0]).tolist()
scale = 1
T = generate_transformation(zyz,translation)
pcd_transformed = copy.deepcopy(transformed_cloud)
...
C_pcd = C_pcd_raw.voxel_down_sample(voxel_size=VOXEL_SIZE)

print('length of src: ', len(A_pcd.points))
print('length of src: ', len(B_pcd.points))
print('length of src: ', len(C_pcd.points))
...
C_xyz = pcd2xyz(C_pcd) # np array of size 3 by M
...
o3d.visualization.draw_geometries([A_pcd_T_teaser,C_pcd])
...
o3d.visualization.draw_geometries([A_pcd_T_icp,C_pcd])
